...
10. In Desktop mode, Hop first adopts a normal clean Git commit as a concurrent baseline when the intended branch tip, attached HEAD, real index, and visible tree all exactly agree on one strict fast-forward descendant of the accepted commit. It otherwise captures ordinary nonignored visible-root edits only from a provable base, then merges either input before materializing `A`. Once that exact accepted tree is proven visible, it may compare-and-swap fast-forward the intended attached branch and atomically replace only a projection-only real index; protected staged/index state, divergence, Git operations, ignored collisions, and synchronization races remain fail-closed.
